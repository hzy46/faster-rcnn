...
class ksz_cyc(imdb):
    def __init__(self, image_set, path=None):
        imdb.__init__(self, 'ksz_cyc_' + image_set)
        self._image_set = image_set
        self._path = self._get_default_path() if path is None \
            else path
        self._img_path = os.path.join(self._path, 'training/training')
        self._label_file = os.path.join(self._path, 'label_cyc.idl')
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'ksz')

    def _get_image_set_file(self):
        return os.path.join(self._path, self._image_set + '_cyc.txt')
    ...
    def _get_sz_results_file_template(self):
        ...
        if self.config['use_salt']:
            ...
            salt = ''
        filename = 'sz_cyc_result_to_evaluation' + salt + self._image_set + '_{:s}.txt'
        path = os.path.join(
            self._path,
            'results',
            filename)
