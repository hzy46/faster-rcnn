...
if __name__ == '__main__':
    ...
    step = 0.1
    if os.path.exists('test_result.txt') is False:
        with open('test_result.txt', 'w'):
            pass
    with open('test_result.txt', 'a') as f:
        while (start <= end):
            cfg.TEST.NMS = start
            result = test_net(rcnn_raw_data_file_list, imdb, vis=args.vis)
            print(result)
            f.write('%f, result: %f \n' % (float(start), float(result)))
            start += step
            f.flush()
